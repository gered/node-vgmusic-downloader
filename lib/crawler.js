...
var pageCrawler = function(page, i) {
  ...
  myCrawler.maxDepth = 10;

  // Crawler Handler (on resource fetch complete)
  myCrawler.on('fetchcomplete', async function(queueItem, responseBuffer, response) {
    ...
    if ("audio/midi" === response.headers['content-type'] || "audio/sp-midi" === response.headers['content-type'])
    {
      console.log(('[#'+i+'] ').grey+("Downloaded " + file_name + " ("+ (responseBuffer.length / 1000)+" kb) in %dms").blue, queueItem.stateData.downloadTime);
      await mkdirp(destination_dir);
      fs.writeFileSync((destination_dir+"/"+file_name), responseBuffer)    
    }
  });
